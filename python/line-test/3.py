...
def left_outer_join(t1, t2):
    ret = []
    for field in t1:
        key = field[0] if field[0] == "id" else int(field[0])
        if key in t2.keys():
            for item in t2[key]:
                field.append(item)
        else:
            for _ in range(len(t2["id"])):
                field.append("NULL")
    fields = t1[1:]
    fields.sort()
    ret.append(" ".join(t1[0]))
    for line in fields:
...
if __name__ == "__main__":
    ...
    for _ in range(int(input())):
        tmp = input().split()
        f_key = tmp[0] if tmp[0] == "id" else int(tmp[0])
        t2[f_key] = tmp[1:]
    for el in left_outer_join(t1, t2):
        print(el)
