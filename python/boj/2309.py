...
주장했다. 뛰어난 수학적 직관력을 가지고 있던 백설공주는, 
...
def seven_dwarfs(heights):
    ret = []
    def find(topick, picked):
        nonlocal ret
        if sum(picked) == 100 and len(picked) == 7:
            ret =  picked[:]
        for i in range(len(topick)):
            picked.append(topick[i])
            find(topick[i+1:], picked)
            picked.pop()
    find(heights, [])
    return ret    
...
if __name__ == "__main__":
    heights = [int(input()) for _ in range(TOTAL_DWARFS)]
    results = seven_dwarfs(heights)
    results.sort()
