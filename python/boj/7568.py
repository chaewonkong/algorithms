...
N명의 집단에서 각 사람의 덩치 등수는 자신보다 더 "큰 덩치"의 
사람의 수로 정해진다. 만일 자신보다 더 큰 덩치의 사람이 k명이라면 
그 사람의 덩치 등수는 k+1이 된다. 
...
키를 나타내는 양의 정수 x와 y가 하나의 공백을 두고 각각 나타난다. 
...
def solution(arr):
    ...
    def bigger(a, b):
        if a.w > b.w and a.h > b.h:
            return 0, 1
        elif a.w < b.w and a.h < b.h:
            return 1, 0
        return 0, 0

    for i in range(N):
        for j in range(i+1, N):
            ret[i] += bigger(arr[i], arr[j])[0]
            ret[j] += bigger(arr[i], arr[j])[1]
    return ret
...
if __name__ == "__main__":
    ...
    data = list()
    for _ in range(N):
        physique = namedtuple('physique', ['w', 'h'])
        a, b = map(int, input().split())
        data.append(physique(w=a, h=b))
    print(" ".join(str(i) for i in solution(data)))