...
function getInfested(n, graph) {
  const infested = [];
  for (let i = 0; i <= n; i++) infested.push(0);
  infested[1] = 1;
  count = 0;

  function setInfested(k) {
    for (let j = 0; j <= n; j++) {
      if (graph[k][j] === 1) {
        infested[j] = 1;
        setInfested(j);
  ...
  setInfested(1);
  for (let l = 0; l < infested.length; l++) {
    if (l > 1 && infested[l] === 1) count += 1;
  }
  return count;
}

console.log(getInfested(n, graph));
